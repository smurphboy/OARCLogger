...
def leaderboards():
    ...
    facts['gridtable'] = db.session.query(User.name, db.func.count(db.distinct(QSO.gridsquare))).filter(and_(func.date(QSO.qso_date) >= '2023-07-01'),(func.date(QSO.qso_date) <= '2023-08-31')).join(Callsign, QSO.station_callsign == Callsign.name).join(User, Callsign.user_id == User.id).group_by(User.name).order_by(db.func.count(db.distinct(QSO.gridsquare)).desc()).limit(10).all()
    facts['totalgrids'] = QSO.query.with_entities(QSO.gridsquare).filter(and_(func.date(QSO.qso_date) >= '2023-07-01'),(func.date(QSO.qso_date) <= '2023-08-31')).distinct().count()
    facts['mygridtable'] = db.session.query(User.name, db.func.count(db.distinct(QSO.my_gridsquare))).filter(and_(func.date(QSO.qso_date) >= '2023-07-01'),(func.date(QSO.qso_date) <= '2023-08-31')).join(Callsign, QSO.station_callsign == Callsign.name).join(User, Callsign.user_id == User.id).group_by(User.name).order_by(db.func.count(db.distinct(QSO.my_gridsquare)).desc()).limit(10).all()
    facts['totalmygrids'] = QSO.query.with_entities(QSO.my_gridsquare).filter(and_(func.date(QSO.qso_date) >= '2023-07-01'),(func.date(QSO.qso_date) <= '2023-08-31')).distinct().count()
